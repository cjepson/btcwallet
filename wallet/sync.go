...
// function below.
var finalAcctScanLength int = 20
...
func (w *Wallet) accountIsUsed(account uint32, chainClient *chain.RPCClient) bool {
	...
	// DEBUG
	log.Infof("Test account %v", account)
	for branch := uint32(0); branch < 2; branch++ {
		for i := uint32(0); i < acctSeekWidth; i++ {
			...
			if err != nil {
				// DEBUG
				log.Infof("Test account %v addr given at idx 0 BAD KEY %v", account, err)
				continue
			}

			if account < 5 && i == 0 {
				// DEBUG
				log.Infof("Test account %v addr given at idx 0 %v", account, addr.EncodeAddress())
			}
			...
			if exists {
				// DEBUG
				log.Infof("Account is used! acct %v", account)
				return true
			}
		}
	}
...
func (w *Wallet) bisectLastAcctIndex(hi, low int) int {
	...
	for i := hi - low - 1; i > 0; i /= 2 {
		if i+offset+int(acctSeekWidth) < waddrmgr.MaxAddressesPerAccount {
			for j := i + offset; j < i+offset+int(acctSeekWidth); j++ {
				// DEBUG
				log.Infof("j pos %v in bisectLastAcctIndex", j)
			...
			if w.accountIsUsed(uint32(i+offset), chainClient) {
				// DEBUG
				log.Infof("i+offset pos %v in bisectLastAcctIndex", i+offset)
...
// findAcctEnd is a helper function for searching for used addresses.
func (w *Wallet) findAcctEnd(start, stop int) int {
	indexStart := w.bisectLastAcctIndex(stop, start)
	indexLast := 0
	for {
		// DEBUG
		log.Infof("indexStart %v, indexLast %v in findAcctEnd", indexStart, indexLast)
	...
	// DEBUG
	log.Infof("indexLast %v in findAcctEnd", indexLast)
...
// function below.
var finalAddrScanLength int = 1000
...
func (w *Wallet) rescanActiveAddresses() error {
	...
	for acct := uint32(0); acct <= lastAcct; acct++ {
		...
		for branch := uint32(0); branch < 2; branch++ {
			...
			log.Infof("DEBUG oldidx %v for acct %v, branch %v", oldIdx, acct, branch)
			if err != nil {

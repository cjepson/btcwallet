...
func (w *Wallet) accountIsUsed(account uint32, chainClient *chain.RPCClient) bool {
	// Search external branch than internal branch for a used
	// address.
	for i := uint32(0); i < acctSeekWidth; i++ {
		for branch := uint32(0); branch < 2; branch++ {
			addr, err := w.Manager.GetAddress(i, account, branch)
	...
	for acct := uint32(0); acct <= lastAcct; acct++ {
		...
		for branch := uint32(0); branch < 2; branch++ {
			...
			if addr == nil && err == nil {
				...
				addr, err := w.Manager.GetAddress(idx,
					waddrmgr.DefaultAccountNum, branch)
				// Skip erroneous keys.
				if err != nil {
					continue

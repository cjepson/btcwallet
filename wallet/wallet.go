...
func newWallet(vb uint16, esm bool, btm dcrutil.Amount, addressReuse bool,
	...
	autoRepair bool, initialSync bool, passphrase []byte, mgr *waddrmgr.Manager,
...
func (w *Wallet) syncWithChain() error {
	...
	// wallet with the provided passphrase.
	if w.initialSync {
		// DEBUG
		log.Infof("Attempting initial unlock")
		var unlockAfter <-chan time.Time
		err := w.Unlock(w.passphrase, unlockAfter)
		if err != nil {
			return err
		}
	}
	// DEBUG
	log.Infof("initial unlock succeed")
	...
	// If this is the initial sync, lock the wallet after completing
	// synchronisation.
	if w.initialSync {
		w.Lock()
	}
...
func Open(pubPass []byte, params *chaincfg.Params, db walletdb.DB, waddrmgrNS,
	...
	ticketMaxPrice float64, autoRepair bool, initialSync bool,
	passphrase []byte) (*Wallet, error) {
